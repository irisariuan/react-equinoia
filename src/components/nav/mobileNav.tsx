    ...
    return (
        <div className="overflow-hidden">
            ...
            <motion.button onClick={activeHandler} className="mx-4 select-none outline-none" id="navMobileBtn" animate={{ transition: { duration: 0.3 }, opacity: [0, 1] }}>
                <FontAwesomeIcon icon={faBars} className="text-2xl lg:text-4xl text-rice-dark " />
            </motion.button>


            <AnimatePresence>
                {active &&
                    <motion.div id="mobileMenu" className="origin-top fixed top-0 left-0 h-screen w-screen bg-rice/80 z-[80] backdrop-blur-3xl overflow-hidden" initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} exit={{ scaleY: 0 }} transition={{ duration: 0.3 }} >
                        <div className="flex items-center justify-end">
                            <motion.button onClick={activeHandler} className="m-4 select-none [&>*]:hover:text-rice-dark/50 outline-none" exit={{ opacity: [1, 0], scaleY: [1, 0], scaleX: [1, 5] }}>
                                <FontAwesomeIcon icon={faCircleXmark} className="text-3xl lg:text-4xl text-rice-dark " />
                            </motion.button>
                        </div>
